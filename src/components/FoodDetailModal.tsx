...
interface FoodDetailModalProps {
  food: TrendingFood | null;
  isOpen: boolean;
  onClose: () => void;
  onStartHangout: () => void;
}

export function FoodDetailModal({ food, isOpen, onClose, onStartHangout }: FoodDetailModalProps) {
  if (!food) return null;
          ...
          >
            ...
            <div className="relative h-48 w-full">
              ...
              <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm rounded-lg px-3 py-1.5 flex items-center gap-2">
                <span className="text-lg">{food.appLogo}</span>
                <span className="text-sm font-medium text-gray-700">{food.appName}</span>
              </div>
            </div>
              ...
              <div className="space-y-3 mb-6">
                <div className="flex items-start gap-3">
                  <div className="p-2 bg-[#D4F4E7]/20 rounded-lg">
                    <UtensilsCrossed className="w-5 h-5 text-[#D4F4E7]" />
                  </div>
                  <div className="flex-1">
                    <p className="text-white/60 text-sm">Cuisine</p>
                    <p className="text-white font-medium">{food.cuisine}</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-white font-medium">{food.location}</p>
                  </div>
                </div>
              </div>
